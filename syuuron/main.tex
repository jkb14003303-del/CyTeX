...
電流遮断時には，接点の開極に伴って高温アークが発生し，開閉器内部に熱的損傷を及ぼすおそれがある．そのため，アークを確実かつ速やかに消弧し，電流を遮断することが求められる．

気中開閉器の種類によっては，内部に多数の金属板を等間隔で積層した消弧室が設置されている．
アークが消弧室に接触すると，消弧板によってアークが分割され，アーク電圧の上昇に伴って導電性が低下する．
また，消弧板とともにポリマー板を積層する場合もあり，ポリマーの溶発によってアークが冷却される．
しかし，これらの消弧手段を講じた場合でも，電流ゼロ点でアークが一度消弧した後に再発弧が生じ，遮断完了が大きく遅延することがある．
...
具体的には，第一電流ゼロ点での遮断に成功したケースでは，スポット位置が可動電極上で接点部（極間）から遠ざかる傾向が顕著であった．
このとき，アークスポット近傍の導電率は，$10^{-1}$～$10^2\,\mathrm{S/m}$ と，同ゼロ点で遮断されなかったケースと比較して約1桁低い値を示した．
これは，スポット距離の増大によりアーク経路が強制的に伸長され，アーク抵抗の上昇に伴って導電率が抑制されたためであり，
その後，電流ゼロ点直後のジュール加熱が低減され，熱的遮断に至ったと推察される．
また，スポット距離が長くなることで，電極間に $4000\,\mathrm{K}$ 未満の低温領域の形成が促進され，絶縁性能も向上したと考えられる．
加えて，可動電極のスポットが Cu-W 製の先端部から電極基部の Cu 材へ遷移したことで，材料間の融点や潜熱の差といった熱的特性の違いが，アーク温度の低下を助長した可能性も示唆された．

第二に，ゼロ点時間が長い条件下では，消弧室右上部に噴出するガスの発光強度が強いケースほど，再発弧が抑制される傾向が認められた．
噴出したガスは発光強度が高いケースほど高温・高導電率な領域が広範囲に分布しており，それに伴いアーク経路の発光は低下していた．
また，電極間に注入された電気エネルギー(アーク電圧×アーク電流の時間積分)は遮断の成否で有意に違わなかった．
したがってアークの熱エネルギーが消弧室外へ効率的に排出されたことで，再発弧が抑制されたと考えられる．
これを踏まえ，アーク経路の温度合計値に対する噴出ガスの温度合計値の比を算出したところ，遮断成功時には再発弧時より二倍以上高い温度比を示した．
なお，ガス噴出が比較的弱い場合であっても，アーク経路自体の発光が十分に抑制されていれば相対的に総和比は増大し，遮断は成功した．

以上の通り，本研究によって，気中開閉器における熱的遮断の成功には
「可動電極上でのアークスポット移動に伴うアーク経路の伸長」と「消弧室から空間への熱輸送によるアーク経路の冷却」という二つのプロセスが極めて重要であることが示された．
...
さらに，線路工事時や自然災害による障害発生時には，停電範囲を最小限に抑えることを目的とした区分開閉の責務をも担う．
...
特に交流電源では，多くの場合，電源周波数の半周期ごとに電流ゼロ点を迎え，この瞬間にはアークプラズマへの注入電力が零となる．
この電流ゼロ点において，アークを消滅あるいは低導電性状態へと移行させる作用が十分に強ければ，電流を遮断することができる\cite{渋谷正豊1987第}．
...
このように，電流ゼロ点近傍の現象を詳細に調査することは，再発弧メカニズムの解明において極めて有用である．\\
　遮断成功に至るためには，熱的および誘電的再発弧の双方を防ぐ必要があり，遮断成功のためには，とりわけ熱的再発弧の抑制が第一関門となる．
...
\begin{table}[H]
  ...
  \caption{開閉機器の比較}
  \label{tab:cb_class}
  \includegraphics[width=0.7\linewidth]{遮断器の表2.png}
\end{table}
...
アークを吹き飛ばすとともに，電流ゼロ点通過時に急速に冷却することでイオンを消滅させ，
...
アーク消弧時の空気流が機外へ排出され，
爆発的な動作音を発生させるという欠点も指摘されている \cite{BN05571456}．\\
...
この現象は熱的破壊と呼ばれる．
ゼロ点近傍におけるアークの時間的挙動は，Mayerの式により評価される．
...
誘電的破壊が起きた場合，
電流ゼロ点直前に電圧が急激に上昇し，消弧ピークを示す．

\begin{figure}[tb]
  \centering
  \includegraphics[width=0.6\linewidth]{電流零点における過渡回復電圧.png}
  \caption{電流ゼロ点における過渡回復電圧}
  \label{fig:電流ゼロ点における過渡回復電圧}
\end{figure}
\FloatBarrier
\clearpage
\section{発光分光測定}
...
電流ゼロ点後に生じる再発弧は，残留電流およびエネルギー注入に起因する熱的再発弧と，
過渡回復電圧の上昇に伴う絶縁領域の誘電的破壊による再発弧に大別される．
これに基づき，本研究では第一電流ゼロ点における遮断成否を，以下の3パターンに分類した．
...
\begin{itemize}
 ...
 \item 誘電的再発弧
\end{itemize}
　図\ref{fig:熱的再発弧特性}に熱的再発弧，図\ref{fig:誘電的再発弧特性}に誘電的再発弧の電流・電圧波形および各時刻（①〜③）における発光画像を示す．
熱的再発弧の場合，電流ゼロ点直後の電圧上昇過程において再発弧が生じるため，過渡回復電圧のピーク値は極めて低い．
また，時刻②において，先行するアーク経路に沿った発光が残留しており，時刻①および③を比較しても，アーク経路に顕著な変化は認められない．
一方，誘電的再発弧では，電流ゼロ点から長い時間を経た後再発弧が生じるため，再発弧時の過渡回復電圧は高い．
発光画像を確認すると，時刻②ではアーク経路に沿った発光が完全に消失しており，再発弧後の時刻③では，時刻①とは異なる経路にアークが形成されている．

以上より，過渡回復電圧のピーク値，再発弧時のアーク経路の変化，および電流ゼロ点近傍における残留発光の有無に着目することで，熱的再発弧と誘電的再発弧を判別した．

表\ref{tab:zc_classification}に，実施した全試験結果について，
ゼロ点時間ごとの再発弧の分類を示す．
なお，ゼロ点時間が $0\sim3\,\mathrm{ms}$ の区間ではギャップ間が短いために，
電極間に形成されるアークの様相を十分に判別することができなかった．
ゼロ点時間が長くなるほど消弧室内におけるアーク冷却時間が長くなるため，
熱的再発弧は抑制される傾向にある．
一方，ゼロ点時間が長くなるほど，冷却の進行に伴い絶縁回復が進むことから，
誘電的再発弧の発生回数は高くなる傾向にある．
しかしながら，誘電的再発弧の発生回数は，
...
\begin{figure}[H]
  ...
  \begin{subfigure}{0.8\linewidth}
    \centering
    \includegraphics[width=\linewidth]{熱的例_IV.png}
    \caption{電流電圧波形}
    \label{fig:熱的_IV}
  \end{subfigure}
  ...
  \begin{subfigure}{0.32\linewidth}
    \centering
    \includegraphics[width=\linewidth]{熱的_発光1.png}
    \caption{①発光}
    \label{fig:熱的発光1}
  \end{subfigure}
  \hfill
  \begin{subfigure}{0.32\linewidth}
    \centering
    \includegraphics[width=\linewidth]{熱的_発光2.png}
    \caption{②発光}
    \label{fig:熱的発光2}
  \end{subfigure}
  \hfill
  \begin{subfigure}{0.32\linewidth}
    \centering
    \includegraphics[width=\linewidth]{熱的_発光3.png}
    \caption{③発光}
    \label{fig:熱的発光3}
  \end{subfigure}

  \caption{熱的再発弧の電流電圧波形と発光画像}
  \label{fig:熱的再発弧特性}
\end{figure}
...
\begin{figure}[H]
  ...
  \begin{subfigure}{0.8\linewidth}
    \centering
    \includegraphics[width=\linewidth]{誘電的例_IV.png}
    \caption{電流電圧波形}
    \label{fig:誘電的_IV}
  \end{subfigure}
  ...
  \begin{subfigure}{0.32\linewidth}
    \centering
    \includegraphics[width=\linewidth]{誘電的_発光1.png}
    \caption{①発光}
    \label{fig:誘電的発光1}
  \end{subfigure}
  \hfill
  \begin{subfigure}{0.32\linewidth}
    \centering
    \includegraphics[width=\linewidth]{誘電的_発光2.png}
    \caption{②発光}
    \label{fig:誘電的発光2}
  \end{subfigure}
  \hfill
  \begin{subfigure}{0.32\linewidth}
    \centering
    \includegraphics[width=\linewidth]{誘電的_発光3.png}
    \caption{③発光}
    \label{fig:誘電的発光3}
  \end{subfigure}

  \caption{誘電的再発弧の電流電圧波形と発光画像}
  \label{fig:誘電的再発弧特性}
\end{figure}

\begin{table}[H]
 \centering
 \caption{第一電流ゼロ点における再発弧の分類}
...
ゼロ点時間が長い場合には，第一電流ゼロ点到達時における電極間距離が十分に確保されるため，
アークが消滅しやすく，遮断成功率が高くなる傾向にある．
一方，ゼロ点時間が短い場合には，
電極間距離が不十分な状態で第一電流ゼロ点を迎えるため，
第一電流ゼロ点で遮断失敗が生じやすい．
...
$15.4\,\mathrm{ms}$ 以上の場合を「長時間遮断」とし，
「短時間遮断」と「長時間遮断」の中間に該当するケースを
「通常遮断」と分類した．

また，詳細は後述するが，
「短時間遮断」に該当するケースのアーク挙動を調べた結果，
第一電流ゼロ点における可動電極上のアークスポット位置が，
第一電流ゼロ点での遮断成否に大きく影響していることが示唆された．
...
遮断時間の分類に基づき，
第一電流ゼロ点付近におけるアーク特性を比較したところ，
可動電極上に形成されるアークスポット位置に明確な差異が確認された．
そこで，図\ref{fig:アークスポット距離}に示すように，第一電流ゼロ点直前における可動電極上のアークスポット位置と
可動電極上端面との最短距離を「アークスポット距離」と定義し，比較を行った．
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.7\linewidth]{スポット距離.png}
  \caption{アークスポット距離の定義}
  \label{fig:アークスポット距離}
\end{figure}
...
\begin{figure}[H]
	...
	\begin{subfigure}[b]{0.48\columnwidth}
		...
		\label{fig:ケースbIV}
	\end{subfigure}
	\caption{第一電流ゼロ点における遮断成否の電流電圧波形(ゼロ点時間は同程度)}
	\label{fig:電流ゼロ点直前の電流と電位差}
\end{figure}

図\ref{fig:アークスポット距離}と同様に，図\ref{fig:電流ゼロ点直前のアーク発光比較}におけるアークスポット距離を算出した．
結果，(a)，(b)それぞれのアークスポット距離は $20.2\,\mathrm{mm}$ および $43.2\,\mathrm{mm}$ であり，
...
導電率が低く抑えられ，電流ゼロ点直後に注入される過渡回復電圧起因のジュールエネルギー $\sigma\,E^{2}$ も小さくなり熱的遮断に成功したものと考えられる．
...
これらの材料特性の差異も，ケース(b)のアーク温度低下に寄与した一因であると考えられる．

\begin{figure}[H]
	\centering
	\begin{subfigure}[b]{0.48\columnwidth}
		\centering
		\includegraphics[width=\linewidth]{ケースaEmission.png}
		\subcaption{ケースa}
	...
	\begin{subfigure}[b]{0.48\columnwidth}
		\centering
		\includegraphics[width=\linewidth]{ケースbEmission.png}
		\subcaption{ケースb}
...
\subsection{ガス発光との分類}
表\ref{tab:遮断性能表}に示した遮断時間の分類に基づき，「短時間遮断」を除くケースの特性を比較した．
その結果，ゼロ点時間が $7\,\mathrm{ms}$ 以上の条件において，第一電流ゼロ点直前における消弧室右上部のガス発光量に明確な差異が認められた．
そこで本研究では，図\ref{fig:ガス測定範囲}に示す白枠領域内において，
発光強度がしきい値 $I_{\mathrm{th}}$ を上回る画素数を
「高発光領域」の指標として定義し，評価を行った．
ここで，$I_{\mathrm{th}}$ は，
同一露光条件下において未飽和であった場合には，
最大発光強度の 70\% とした．
さらに，図\ref{fig:発光強度分類境界}に示すように，
高発光領域の画素数を計測範囲内の全画素数で除して規格化した値を算出した．
本研究では，この規格化値が 0.5\% 以下の場合を「小」，
3.5\% 以上の場合を「大」として発光規模を分類・評価した．
なお，分類には鉄の発光スペクトルのうち，
$640\,\mathrm{nm}$ よりも撮影範囲が広い $650\,\mathrm{nm}$ の発光画像を用いた．

表\ref{table:ガス発光表}に，ゼロ点時間ごとの発光強度の分類結果を示す．
表\ref{table:ガス発光表}より，ゼロ点時間が $7\,\mathrm{ms}$ 以上の条件下において，発光強度が「大」に分類された割合に着目すると，
第一電流ゼロ点での遮断成功ケースは17回中12回（約71$\%$）であったのに対し，再発弧したケースは11回中2回（約18$\%$）にとどまり，両者の間には明確な差異が確認された．
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.7\linewidth]{発光強度分類境界.png}
  \caption{発光強度の分類}
  \label{fig:発光強度分類境界}
\end{figure}
\begin{table}[H]
...
また，電極間に注入された電気エネルギー(アーク電圧×アーク電流の時間積分)は遮断の成否で有意に違わなかった．
したがって消弧室右上部へ噴出したガスが，高温・高導電率で強い発光を伴っていることは，アークの熱エネルギーが消弧室から効率的に排出されたことを意味する．
...
なお実験番号は図\ref{fig:発光強度分類境界}と同一である．
...
ここで，No.7 以降のケースでは，前述の通り消弧室右上部におけるガス発光の増大に起因して総和比も増大したと考えられる．
一方，ガス発光が比較的弱いにもかかわらず第一電流ゼロ点で遮断に成功した No.2 および No.4 では，図\ref{fig:No.3,No.6の発光画像}に示すように，
電流ゼロ点直前におけるアーク経路自体の発光が十分に弱く，相対的に総和比が増大し，遮断成功に至ったものと推察される．
以上より，ゼロ点時間が $7\,\mathrm{ms}$ 以上の遮断においては，消弧室右上部への高エネルギー熱ガスの効率的な排出，
あるいはアーク経路の発光が十分に抑制されることが，遮断成否を分ける要因と考えられる．
...
\begin{itemize}
 ...
 \item 開極から第一電流ゼロ点までの時間が長い場合，消弧室右上部におけるガス発光強度が強いほど，遮断成功確率が高くなる傾向が確認された．
  \vspace{\baselineskip}
 \item 遮断成功時には，消弧室右上部における高温・高導電率のガス空間が遮断失敗時の約1.8倍に拡大し，アーク経路の温度合計値に対する噴出ガスの温度合計値の比は再発弧時より二倍以上高い比を示した．
  \vspace{\baselineskip}
 \item 電極間に注入された電気エネルギーは遮断の成否で有意に違わなかったため，ガス発光が強くなることでアーク経路の発光が抑制されると考えられる．
\vspace{\baselineskip}
