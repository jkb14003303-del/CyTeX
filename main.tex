...
遮断時には，接点の開極に伴い高温アークが発生するため，これを確実かつ速やかに消弧することが求められる．
しかし，一度消弧したアークが再発弧すると，遮断完了が遅延し，火災などの重大事故を引き起こすおそれがある．
再発弧には，残留高温プラズマに起因する熱的再発弧と，絶縁破壊による誘電的再発弧が存在し，特に発生頻度の高い熱的再発弧の抑制が重要であるものの，その発生要因は十分に解明されていない．
そこで本研究では，熱的遮断の成否が決定づけられる電流ゼロ点近傍における物理現象の解明を目的とし，分光画像センサーを用いたアーク温度および導電率の二次元分布測定を行った．
具体的には，Fe I 線を用いた二線強度比法によりアーク温度を算出し，空間粒子組成および Fe I–O I 線強度比に基づいて鉄蒸気混入率ならびに導電率を算定した．
また，開極から第一電流ゼロ点までの時間を「ゼロ点時間」と定義し，ゼロ点時間ごとに条件を切り分けて評価を行った．
解析の結果，遮断成否を支配する要因として，以下の二つのメカニズムが明らかとなった．
第一に，ゼロ点時間が短く，物理的な電極間距離が不十分な条件下であっても，可動電極上のアークスポットが電極先端から遠方に形成される場合には，遮断が成功しやすい傾向が確認された．
これは，スポット距離の増大によってアーク経路が強制的に伸長され，アーク抵抗が増大するためである．
その結果，ゼロ点直前におけるアーク先端の温度および導電率が低下し，過渡回復電圧によるジュール加熱が抑制されることで，熱的遮断が達成される．
さらに，このスポット移動は低温の絶縁層形成を促進し，誘電的遮断にも寄与する可能性が示唆された．
第二に，ゼロ点時間が長い条件下では，消弧室右上部から噴出するガスの発光強度が強いケースほど，再発弧が抑制される傾向が認められた．
解析の結果，同領域のガスは，発光強度が高いほど高温かつ高導電率の空間が拡大しており，また遮断成功例では，アーク経路の温度に対する噴出ガスの温度比が大きいことが確認された．
このことから，ガス排出に伴いアークからエネルギーが奪われ，冷却が促進されていると考えられる．
以上より，本気中開閉器における熱的遮断の成功には，電極上のアークスポット移動によるアーク経路の伸長と，消弧室からの高温ガス排出およびそれに伴う冷却プロセスが極めて重要であることが示された．
...
\begin{enumerate}
    \item \textbf{断路器（Disconnector）}：電路の接続切り替えや保守時の隔離を目的とし，無電流あるいはそれに極めて近い状態で電路を開閉する。
    \item \textbf{負荷開閉器（Load Switch）}：定格負荷電流や過負荷電流程度の開閉は安全に行えるが，短絡事故等に伴う故障電流の遮断能力は有さない。
    \item \textbf{遮断器（Circuit Breaker）}：常時電流のみならず，過酷な故障電流をも支障なく遮断できるように設計されており，系統の保護用装置として機能する。
\end{enumerate}
...
\subsection{ガス発光との分類}
\ref{sec:遮断時間の分類}に示した遮断時間の分類に基づき，
「短時間遮断」を除くケースの特性を比較したところ，
ゼロ点時間が $7\,\mathrm{ms}$ 以上の場合に，
第一電流ゼロ点直前における消弧室右上部のガス発光量に
明確な差異が確認された．
そこで本研究では，鉄の発光線のうち，
640\,nm に比べて発光強度が弱い波長 650\,nm の発光画像に着目した。
図\ref{fig:ガス測定範囲}に示す白枠領域内において，
発光強度がしきい値 \(I_{\mathrm{th}}\) を上回る画素数を，
高発光領域の指標として評価した。
ここで \(I_{\mathrm{th}}\) は，
同一露光条件下において未飽和であった場合には最大発光強度の 75\%，
一方，飽和が大幅に生じた露光条件においては撮像素子の飽和値と定義した。
同一実験条件下における発光強度分布の一例を図\ref{fig:発光強度分類境界}に示す．
図\ref{fig:発光強度分類境界}のように、閾値以上の画素数から、「小」「中」「大」の三段階に分類した．
...
また，
波長650\,nmにおける発光強度分布に，
同時刻の可動電極位置を模式的に重ねて示した結果を
図\ref{fig:電流ゼロ点直前のアーク発光比較2}に，
温度および導電率分布を
図\ref{fig:電流ゼロ点直前のアーク温度・導電率比較2}に示す．

図\ref{fig:電流ゼロ点直前の電流と電位差2}に示すとおり，
第一電流ゼロ点を基準時刻 $t = 0\,\mathrm{ms}$ とした．
各ケースのゼロ点時間は，
それぞれ $8.27\,\mathrm{ms}$ および $8.32\,\mathrm{ms}$ である．

図\ref{fig:電流ゼロ点直前のアーク発光比較2}(c)，(d)より，　
消弧室右上部の発光強度は，
ケース(c)では約 $30000$～$40000$ の領域が多く分布しているのに対し，
ケース(d)では約 $50000$ と，
より強い発光が確認される．
また，
同領域において
図\ref{fig:電流ゼロ点直前のアーク温度・導電率比較2}(c1)，(d1)に示す温度分布を比較すると，
ケース(d)はケース(c)に比べて
$8000\,\mathrm{K}$ 前後の高温領域が多く，
全体的に高温となっていることが分かる．
具体的には，
$8000\,\mathrm{K}$ 以上の領域の画素数は，
ケース(c)で296\,px，
ケース(d)で527\,pxであった．

さらに，
図\ref{fig:電流ゼロ点直前のアーク温度・導電率比較2}(c2)，(d2)に示す
導電率分布においても，
ケース(d)はケース(c)に比べて
$10^{3}$～$10^{4}\,\mathrm{S/m}$ の領域が多く分布しており，
それぞれ326\,px，592\,pxであった．\
また，白枠で示した消弧室内におけるアーク経路を比較すると，
消弧室右上枠内のガス発光が弱いケース(c)では，消弧室全体にわたって
アーク経路が確認できるのに対し，
ガス発光の強いケース(d)では，特に可動電極近傍において発光が確認できず，
温度および導電率が測定不可能なほど
低温・低導電率の状態となっている。
これらの枠内消弧室右上のガスは，消弧室から噴き出したものであり，
発光が強く、温度・導電率も高いガスが放出されたことで，アーク経路の温度・導電率が低くなり，再発弧が抑制されたと考えられる．\

これらを踏まえ，ゼロ点時間が 7,ms 以上で，かつ温度測定が可能であった 10 ケースを対象として，消弧室右上枠内およびアーク経路におけるガス温度および導電率の総和比を算出した．
その結果を図\ref{fig:温度・導電率総和比}に示す．
実験番号は図\ref{fig:発光強度分類境界}と同一であり，No.1 および No.5 はゼロ点時間が \qty{7}{\milli\second} 以下であったため解析対象から除外した．
...
ここで，No.9 以降のケースでは，消弧室右上部におけるガス発光の増大に起因して総和比が大きくなったと考えられる．
一方，ガス発光が比較的弱いにもかかわらず遮断に成功した No.3 および No.6 では，図に示すように，電流ゼロ点直前におけるアーク経路の発光が弱かったことにより，結果として総和比が増大し，
遮断成功に至ったものと考えられる．
...
\begin{figure}[htbp]
	...
	\begin{subfigure}[c]{0.48\columnwidth}
		\centering
		\includegraphics[width=0.8\linewidth]{ケースcEmission.png}
		\caption*{(c) ケースc}
	...
	\begin{subfigure}[d]{0.48\columnwidth}
		\centering
		\includegraphics[width=0.8\linewidth]{ケースdEmission.png}
		\caption*{(d) ケースd}
		\label{fig:ケースdEmission}
	\end{subfigure}
	\caption{c,dにおける電流ゼロ点直前の電流と電位差}
...
\begin{figure}[htbp]
	...
	\begin{subfigure}[c]{0.48\columnwidth}
		\centering
		\includegraphics[width=0.8\linewidth]{温度比.png}
	...
	\begin{subfigure}[d]{0.48\columnwidth}
		\centering
		\includegraphics[width=0.8\linewidth]{導電率比.png}
